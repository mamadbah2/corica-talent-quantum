...
export default function EmployeeDashboard() {
    ...
    const handleResetSubmittedObjectives = () => {
        setNcKpis([]);
        setMgrKpis({ performance: [], leadership: [] });
        setWorkflowStep('CO_CONSTRUCTION');
        setObjectiveDrafts([{ id: 1, name: '', desc: '', measure: '', deadline: '', weight: 0 }]);
        setEmployeeObjectiveComments('');
        if (currentUser) {
            localStorage.removeItem(`kpi_submitted_${currentUser.id_usercount}`);
        }
        setToastMessage('Objectifs réinitialisés. Vous pouvez en saisir de nouveaux.');
    ...
    const [objectiveDrafts, setObjectiveDrafts] = useState([
        { id: 1, name: '', desc: '', measure: '', deadline: '', weight: 0 }
    ]);
    ...
    const handleSubmitEvaluation = () => {
        if (currentUser) {
            ...
            if (managerN1) {
                addNotification(managerN1.id_usercount, `L'employé ${currentUser.nom_prenoms} a soumis son auto-évaluation.`, "info", "/manager");
            }
        }
        ...
        <div className="flex flex-col h-screen bg-[#E3E1DB] font-sans overflow-hidden">
            ...
            <header className="sticky top-0 z-[100] h-[76px] bg-white border-b border-[#A39D98]/30 px-8 flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-4 text-[13px] font-medium">
                    <select
                        value={grade} onChange={(e) => setGrade(e.target.value as EmployeeGrade)}
                        className="bg-[#E3E1DB] border border-[#A39D98] text-[#463738] rounded px-2 py-1 outline-none font-bold"
                    >
                        <option value="NON_CADRE">Profil: Non-Cadre (Ouvrier/Officer)</option>
                        <option value="MANAGER">Profil: Manager</option>
                    </select>
                    <DownloadGuideButton />
                    <NotificationBell />
                    <div className="flex items-center gap-2">
                        <UserAvatar nom={currentUser?.nom_prenoms ?? 'Employé'} size={34} textClassName="text-xs" />
                        <span className="text-[#463738] font-medium">{currentUser?.nom_prenoms ?? 'Employé A.'}</span>
                    </div>
            ...
            <div className="bg-white px-8 py-3 flex items-center justify-between gap-4 border-b border-[#A39D98]/30 shadow-sm shrink-0">
                <div className="flex items-center gap-2 text-[13px] font-bold text-[#A39D98]">
                    <span className="flex items-center gap-1.5 cursor-pointer hover:text-[#F26322] transition-colors" onClick={() => setActiveView(null)}>
                        <Home size={15} className="mb-0.5" /> 🏠 Page d'accueil Employé
                    </span>
                    {activeView && (
                        <>
            ...
            <main className="flex-1 overflow-y-auto px-4 sm:px-10 py-8 max-w-[1200px] mx-auto w-full">
                ...
                <div className="mb-10 animate-in slide-in-from-top-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {(() => {
                            const isKpiValidated = typeof window !== 'undefined' && localStorage.getItem(`kpi_validated_${currentUser?.id_usercount}`) === 'true';
                            const rawPeriod = typeof window !== 'undefined' ? localStorage.getItem('eval_period') : null;
                            const evalPeriod = rawPeriod ? JSON.parse(rawPeriod) as { startDate: string; endDate: string; label: string } : null;
                            const today = new Date();
                            today.setHours(0, 0, 0, 0);
                            const isPeriodOpen = evalPeriod
                                ? today >= new Date(evalPeriod.startDate) && today <= new Date(evalPeriod.endDate)
                                : false;
                            const isPeriodFuture = evalPeriod ? today < new Date(evalPeriod.startDate) : false;
                            const isPeriodPast = evalPeriod ? today > new Date(evalPeriod.endDate) : false;
                            const canSelfEvaluate = isKpiValidated && isPeriodOpen;

                            let lockMsg = '';
                            if (!isKpiValidated) lockMsg = '⏳ En attente de validation de vos objectifs par votre N+1';
                            else if (!evalPeriod) lockMsg = '🔒 Aucune période d\'auto-évaluation configurée';
                            else if (isPeriodFuture) lockMsg = `🔒 Période s'ouvre le ${new Date(evalPeriod.startDate).toLocaleDateString('fr-FR')}`;
                            else if (isPeriodPast) lockMsg = '🔒 Période d\'auto-évaluation clôturée';

                            return (
                                <div className="relative">
                                    <button
                                        onClick={() => canSelfEvaluate && setActiveView('EVALUATION')}
                                        disabled={!canSelfEvaluate}
                                        className={`w-full p-6 rounded-lg font-bold text-[17px] flex items-center justify-center gap-4 transition-all ${canSelfEvaluate ? 'hover:-translate-y-1 shadow hover:shadow-md cursor-pointer' : 'opacity-60 cursor-not-allowed'}`}
                                        style={{ background: canSelfEvaluate ? 'linear-gradient(to right, #50443b, #9e9e5e)' : 'linear-gradient(to right, #8a8a8a, #5a5a5a)', color: 'white' }}
                                        title={!canSelfEvaluate ? lockMsg : ''}
                                    >
                                        {canSelfEvaluate ? <CheckCircle size={24} /> : <Lock size={24} />}
                                        Mon auto-évaluation
                                    </button>
                                    {!canSelfEvaluate && (
                                        <div className="mt-2 flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                                            <span className="text-[11px] text-amber-700 font-medium leading-tight">{lockMsg}</span>
                                        </div>
                                    )}
                                </div>
                            );
                ...
                {activeView === 'PROFILE' && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#463738]/30 overflow-y-auto animate-in fade-in duration-300">
                        <div className="w-full max-w-4xl bg-[#E3E1DB] rounded-xl p-8 border border-white relative shadow-2xl my-auto">
                            ...
                            <div className="flex flex-col gap-2 mb-6 border-b border-[#A39D98]/20 pb-4">
                                <div className="flex items-center gap-2 text-[12px] font-bold text-[#A39D98]">
                                    <span className="hover:text-[#F26322] cursor-pointer flex items-center gap-1 transition-colors" onClick={() => setActiveView(null)}><Home size={14} className="mb-0.5" /> 🏠 Page d'accueil Employé</span>
                                    <ChevronRight size={14} />
                                ...
                                <div className="bg-white rounded-xl border-[1.5px] border-[#F26322] p-6 shadow-sm relative">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
                                        <div>
                                            ...
                                            <input readOnly value={currentUser ? `${currentUser.id_usercount}` : '10057'} className="w-full bg-[#f6f6f6] border-none rounded-md px-4 py-2.5 text-[#463738] font-medium text-[15px] focus:ring-1 focus:ring-[#F26322] transition-all outline-none" />
                                        </div>
                                        <div>
                                            <label className="text-[12px] font-bold text-[#A39D98] mb-1.5 block">Nom et Prénoms</label>
                                            <input defaultValue={currentUser?.nom_prenoms ?? 'Anzoumana Diakite'} className="w-full bg-[#f6f6f6] border-none rounded-md px-4 py-2.5 text-[#463738] font-medium text-[15px] focus:ring-1 focus:ring-[#F26322] transition-all outline-none" />
                                        </div>
                                        <div>
                                            <label className="text-[12px] font-bold text-[#A39D98] mb-1.5 block">Date de Naissance</label>
                                            <input defaultValue="Non définie" className="w-full bg-[#f6f6f6] border-none rounded-md px-4 py-2.5 text-[#463738] font-medium text-[15px] focus:ring-1 focus:ring-[#F26322] transition-all outline-none" placeholder="JJ/MM/AAAA" />
                                        </div>
                                        <div>
                                            <label className="text-[12px] font-bold text-[#A39D98] mb-1.5 block">Usercount (Email)</label>
                                            <input readOnly value={currentUser?.usercount ?? 'Employe.10057@company.com'} className="w-full bg-[#f6f6f6] border-none rounded-md px-4 py-2.5 text-[#463738] font-medium text-[15px] focus:ring-1 focus:ring-[#F26322] transition-all outline-none" />
                                        </div>
                                        <div>
                                            <label className="text-[12px] font-bold text-[#A39D98] mb-1.5 block">Numéro de Téléphone</label>
                                            <input defaultValue="+225 01 02 03 04 05" className="w-full bg-[#f6f6f6] border-none rounded-md px-4 py-2.5 text-[#463738] font-medium text-[15px] focus:ring-1 focus:ring-[#F26322] transition-all outline-none" placeholder="+Indicatif Numéro" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white rounded-xl border border-[#9A9750] p-6 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
                                        ...
                                        <div>
                                            ...
                                            <input defaultValue="Non définie" className="w-full bg-[#f6f6f6] border-none rounded-md px-4 py-2.5 text-[#463738] font-medium text-[15px] focus:ring-1 focus:ring-[#9A9750] transition-all outline-none" placeholder="JJ/MM/AAAA" />
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            <label className="text-[12px] font-bold text-[#A39D98] mb-1.5 block">Service / Scope d'Affectation</label>
                            ...
                            <div className="mt-8 pt-4 flex justify-end gap-3 border-t border-[#A39D98]/20">
                                ...
                                <button onClick={handleSaveProfile} className="px-5 py-2.5 bg-[#F26322] text-white rounded-md font-bold text-[13px] shadow-sm hover:bg-orange-600 transition-colors flex items-center gap-2">
                                ...
                                <div className="flex gap-3 justify-end">
                                    ...
                                    <button onClick={() => {
                                        if (currentUser) {
                                            ...
                                            localStorage.setItem(`kpi_data_${currentUser.id_usercount}`, JSON.stringify(objectiveDrafts));
                                            
                                            if (managerN1) {
                                                addNotification(managerN1.id_usercount, `L'employé ${currentUser.nom_prenoms} a soumis ses objectifs pour validation.`, "info", "/manager");
                                            }
                                        }
                                        setToastMessage("Objectifs soumis au Manager pour validation.");
                                        setTimeout(() => setToastMessage(null), 3000);
                                        setTimeout(() => setActiveView(null), 3500);
                                    }} className="px-5 py-2 bg-[#9A9750] text-white rounded font-bold text-[13px] shadow-sm hover:bg-[#8d8a49] transition-colors flex items-center gap-2">
                ...
                {activeView === 'EVALUATION' && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#463738]/30 overflow-y-auto animate-in fade-in duration-300">
                        <div className="w-full max-w-[1500px] bg-[#f3f2ef] rounded-xl pt-2 shadow-2xl relative my-auto">
                            ...
                            <div className="bg-white border-b border-[#E3E1DB] overflow-hidden overflow-x-auto">
                                {/* Table Header */}
                                            ...
                                            <div key={kpi.id} className="grid grid-cols-[1.2fr_3fr_3fr_110px_160px_180px_240px] bg-white items-stretch hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <div className="flex gap-1 items-center justify-center">
                                                        {[1, 2, 3, 4].map((star) => (
                                                            ...
                                                            >
                                                                <Star
                                                                    size={20}
                                                                    fill={score >= star ? "currentColor" : "currentColor"}
                                                                />
                                                            </button>
                                                        ))}
                                                    </div>
                                                ...
                                                <div className="p-3 border-r border-[#E3E1DB] flex flex-col items-center justify-center">
                                                    <label className={`w-full h-full min-h-[70px] border-2 border-dashed ${evalFiles[kpi.id] ? 'border-[#9A9750] bg-[#E9EBE2]' : 'border-[#d1d1d1] bg-slate-50'} rounded hover:bg-[#E9EBE2] cursor-pointer transition-colors flex flex-col items-center justify-center p-2 text-center group`}>
                                                        {evalFiles[kpi.id] ? (
                                                            <>
                                                                <FileText size={18} className="text-[#9A9750] mb-1" />
                                                                <span className="text-[9px] text-[#7A863B] font-bold uppercase tracking-wider leading-tight truncate w-full px-1">{evalFiles[kpi.id]?.name}</span>
                                                            </>
                                                        ) : (
                                                            <>
                                                                <UploadCloud size={18} className="text-[#A39D98] mb-1 group-hover:text-[#9A9750] transition-colors" />
                                                                <span className="text-[9px] text-[#A39D98] font-bold uppercase tracking-wider group-hover:text-[#7A863B] transition-colors leading-tight">Joindre Fichier</span>
                                                            </>
                                                        )}
                                                        <input type="file" className="hidden" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" onChange={(e) => handleFileChange(kpi.id, e.target.files?.[0] || null)} />
                                                    </label>
                                                </div>
                                                <div className="p-3 flex items-center">
                                                    <textarea
                                                        value={evalComments[kpi.id] || ''}
                                    ...
                                    return (
                                        <>
                                            <div>
                                                ...
                                                <div className="text-[24px] font-black text-[#F26322] flex items-center gap-2">
                                                    {avgScore}/4 <Star size={20} className="fill-[#F26322]" />
                                                </div>
                                            </div>
                            ...
                            {/* Footer Totals and Actions */}
                            <div className="mt-6 pt-4 border-t border-[#A39D98]/30 flex flex-col md:flex-row justify-between items-center gap-4 bg-[#E2E1D9] rounded-b-xl px-6 py-4">
                                <div className="text-[13px] text-[#8c847e] font-medium">
...

                {
                    activeView === 'DASHBOARD' && (
                        <div className="fixed inset-0 z-50 flex flex-col items-center justify-start py-8 px-4 bg-[#463738]/30 overflow-y-auto animate-in fade-in duration-300">
